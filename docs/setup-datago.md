...
| `미국 0/6` | Stooq 접근 실패 — 일시적 한도 초과일 수 있어 재시도 |

## 3. 종목을 바꾸려면
`config/sources.yaml`의 `stocks:` 블록만 고치면 됩니다. 코드 수정 불필요.
국내는 6자리 단축코드, 미국은 티커(자동으로 `.us`가 붙습니다). 표시는 국내 6·미국 5종목까지.

## 4. 기준 시점
둘 다 **전일 종가**입니다. 다이제스트가 07:30 KST에 나가므로 국내는 어제 장,
미국은 한국시간 새벽에 끝난 직전 정규장이 기준입니다. 장중 시세가 아닙니다.
