...
- [ ] (선택) 같은 포털에서 **"금융위원회_주식시세정보"(15094808) 활용신청** — 다이제스트 머리의 건설주 시세
  - 키는 `DATA_GO_KR_KEY`를 그대로 재사용하지만 **활용신청은 서비스별로 따로** 해야 합니다
  - 신청 전에도 미국 종목과 공고·뉴스는 정상 동작하고 국내 종목만 빠집니다
  - 확인: Run workflow → command: **smoke-stocks**
- [ ] (선택) 미국 종목까지 실으려면 **finnhub.io 무료 가입 → `FINNHUB_API_KEY` Secret 등록** (2분, 분당 60회)
...
1. 스테이징 채널에 쌓인 다이제스트 확인 (매일 아침 07:30 한 통)
